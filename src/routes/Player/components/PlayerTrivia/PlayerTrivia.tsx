...
const PlayerTrivia = ({ round, result, width, height }: PlayerTriviaProps) => {
  ...
  if (isScoreboard) {
    return (
      <div style={{ width, height }} className={styles.container}>
        <div className={clsx('silkscreen', styles.label)}>scores</div>
  ...
  return (
    <div style={{ width, height }} className={styles.container}>
      <div className={clsx('silkscreen', styles.label)}>
        {`${result ? 'answer' : 'trivia'} — ${round.questionNumber} of ${round.questionCount}`}
      </div>
